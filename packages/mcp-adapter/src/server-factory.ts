...
export function createHoplaneMcpServer(backend: HoplaneMcpBackend): McpServer {
  const server = new McpServer({ name: "hoplane", version: "0.1.4" });
  ...
  server.registerTool("execute_command", {
    ...
    description: "Execute a policy-approved, non-interactive command on a specific SSH host. When the host's proxyEnabled field is true, Hoplane requires its reverse tunnel to be active and injects ALL_PROXY automatically; do not add or persist proxy configuration yourself. sudo IS supported when the host's `sudo.available` (from list_hosts) is true: just use `sudo` anywhere in the command, including several chained invocations (e.g. `sudo apt-get update && sudo apt-get install -y nginx`) — Hoplane authenticates every sudo invocation automatically, so never ask the user for a password, never embed one in the command, and never use workarounds like `su` or permission changes. If `sudo.available` is false, sudo commands are rejected; ask the user to enable sudo in the Hoplane app instead.",
    inputSchema: {
