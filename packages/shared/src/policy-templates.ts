...
export const POLICY_TEMPLATES: readonly PolicyTemplate[] = [
  ...
  { key: "container-readonly", name: "容器排障（只读）", scenario: "Docker 容器与 Kubernetes 工作负载排查", description: "允许 Docker 与 Kubernetes 查询，黑名单同时阻止容器和集群变更。", risk: "LOW", recommended: false, document: base(blacklist(...commonMutation, "docker-change", "docker-danger", "k8s-change", "k8s-danger")) },
  { key: "container-operations", name: "容器运维（受限）", scenario: "Docker 启停与 Kubernetes 应用重启、扩缩容", description: "允许 Docker 和 Kubernetes 常规运维，黑名单阻止远程执行、删除、构建和资源写入等高风险操作。", risk: "MEDIUM", recommended: false, document: base(blacklist(...commonMutation, "docker-danger", "k8s-danger")) },
