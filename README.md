...
Hoplane 是一个面向 AI Agent 的本地 SSH 网关与桌面运维工具。它把主机管理、本地加密凭据、Policy V4、操作审计、文件传输、主机级跳板连接与 SOCKS5 反向代理、AI 指令记录和人工交互终端放在同一个 App 中，并可一键接入 Codex、Cursor、Claude Code、WorkBuddy、Trae 与 ZCode。
...
进入顶部“接入”页。Codex、Cursor、Claude Code、WorkBuddy、Trae 和 ZCode 推荐使用一键 stdio 集成；其他支持 Streamable HTTP MCP 的客户端使用“其他 Agent”。

![Hoplane Agent 接入](docs/images/hoplane-integrations.png)

#### Codex、Cursor、Claude Code、WorkBuddy、Trae 和 ZCode
...
```text
Codex / Cursor / Claude Code / WorkBuddy / Trae / ZCode / Other Agents
                         │
                   stdio / HTTP MCP
                         │
                         ▼
...
- 新增主机后自动发起连接测试；
- 支持从本地 SSH Config 导入主机：在「导入配置」弹窗的“本地 SSH 配置”模式下预览解析结果、勾选主机并选择分组；支持 Include 递归展开与 ProxyJump 首跳自动映射为跳板机，导入后默认不开放 AI 访问；
...
| ZCode | `~/.zcode/skills/hoplane` | `~/.zcode/cli/config.json`（`mcp.servers` 嵌套键） |

Trae 国内版使用 `~/.trae-cn/skills/hoplane`，并会扫描对应的 `Trae CN/User/mcp.json`。TRAE SOLO 不单独显示为接入项，而是在 Trae 标签的配置目录列表中显示为 `TRAE SOLO/User/mcp.json`，并复用 `~/.trae/skills/hoplane`。Windows 与 Linux 分别从 `%APPDATA%` 和 `$XDG_CONFIG_HOME`（默认 `~/.config`）下的 Trae 用户配置目录读取。Trae 配置会按其官方格式写入 `command`、`args` 与超时环境变量。ZCode 配置写入用户级 `~/.zcode/cli/config.json` 的 `mcp.servers` 嵌套键（与其余 Agent 的顶层 `mcpServers` 不同），`plugins`、`hooks` 等其他键保持不变。
...
- SSH Config 导入会跳过 `Match` 块；`ProxyJump` 仅取首跳并映射为跳板机，不支持 `ProxyCommand` 与多级跳；不支持 Host 块外的全局指令继承与通配符继承，Include 通配仅支持 `*` 与 `?`；
