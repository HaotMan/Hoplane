...
> 当前版本：`0.1.4`。项目处于 MVP 阶段，安装包尚未进行正式代码签名或公证。
...
顶部导航依次是“主机”“策略”“审计”“接入”；右侧显示本地 Core 状态和外观切换图标。下面的截图全部来自当前界面，**红框表示需要点击、填写或重点查看的位置**。

> 截图使用独立演示数据和 [RFC 5737](https://datatracker.ietf.org/doc/html/rfc5737) 保留地址，不包含真实服务器、Token 或凭据信息。

### 1. 创建和解锁本地保险库

首次启动时，Hoplane 会要求创建本地主密码。主密码用于派生保险库加密密钥，不会写入磁盘。

![创建 Hoplane 本地保险库，红框标出主密码表单和创建按钮](docs/images/hoplane-vault.jpg)

1. 在红框中的“主密码”和“确认主密码”输入相同内容，长度至少 10 个字符；
2. 点击“创建并解锁”；
...
主机页是日常操作入口。每台主机都可以独立设置登录用户、权限策略、AI 访问和文件传输开关。

![Hoplane 主机总览，红框标出导航、添加主机、筛选栏、主机台账、快捷开关和行尾操作图标](docs/images/hoplane-hosts.jpg)
...
- 主机台账：按分组查看主机、连接状态、AI 访问、当前用户和策略；主机名右侧的“启用主机”“启用sudo”“文件传输通道”滑块可以直接控制对应能力；连接状态右侧可以直接测试连接；
...
![添加 SSH 主机，红框标出基本信息、认证方式和保存按钮](docs/images/hoplane-add-host.jpg)
...
3. 非 `root` 用户如需 sudo，可选择仅使用远端 `NOPASSWD`、复用 SSH 登录密码或使用独立 sudo 密码；
4. 选择权限策略。新主机默认推荐“错误追溯（推荐）”；
5. 按需填写默认工作目录和标签；
6. 确认三个主机级开关：
   - “启用主机”默认开启；
   - “允许 AI 访问”默认关闭；
   - “允许主机间文件传输”默认关闭；
7. 点击红框中的“保存主机”。保存成功后，Hoplane 会自动执行一次连接测试。

首次连接或服务器指纹发生变化时，App 会显示 SSH SHA-256 指纹确认框。请从可信渠道核对指纹后再确认；**指纹变化时不要直接信任**。MCP、CLI 和 Agent 都不能代替用户接受指纹。

> “未连接”表示当前 Core 没有正在复用的 SSH 连接，不等于主机故障。连接测试完成后测试连接会主动关闭，因此成功时列表仍可能显示“未连接”；应以行内“连接成功 · xx ms”结果为准。
...
主机启停、sudo 和主机间文件传输使用主机名右侧的滑块控制；连接测试位于当前连接状态右侧。当前用户选择框内的绿色 `SSH` 标识表示这里切换的是 SSH 登录身份，策略选择框内的绿色“权限策略”标识表示这里切换 AI 权限策略。其余操作图标直接位于每条主机信息的最右侧，不再展开额外操作行。鼠标悬浮或使用键盘聚焦图标时，会显示对应的文字提示。“指令记录”使用带回退箭头的时钟图标。

![Hoplane 主机行内操作，红框标出快捷开关、下拉控件和主机信息末尾的操作图标](docs/images/hoplane-operations.jpg)
...
人工终端与 AI 操作是两条不同通道：人工终端不检查 AI 访问开关，也不经过 AI 策略黑名单，但主机必须启用，保险库、凭据和 SSH 指纹必须有效。终端只审计会话开始、结束、耗时、退出码或错误，不保存用户输入和终端输出。建议先“测试连接”并确认指纹，再打开终端。
...
![Hoplane 权限策略页面，红框标出策略工具、策略列表和编辑方式](docs/images/hoplane-policies.jpg)
...
![Hoplane 操作审计，红框标出结果筛选和审计表格](docs/images/hoplane-audit.jpg)
...
![Hoplane Agent 接入，红框标出 Agent 标签、本地 HTTP 服务和连接配置](docs/images/hoplane-integrations.jpg)
